...
def get_stock_data(symbol, start_date=None, end_date=None, period="daily"):
    ...
    try:
        # 获取A股数据 - 使用股票代码直接获取
        stock_df = ak.stock_zh_a_hist(symbol=symbol, period=period, 
...
def generate_signals(data):
    ...
    # 1. MA金叉策略
    ma_cross = False
    if pd.notnull(prev['MA5']) and pd.notnull(prev['MA20']) and pd.notnull(latest['MA5']) and pd.notnull(latest['MA20']):
    ...
    if pd.notnull(latest['RSI']):
        rsi_oversold = latest['RSI'] < 35  # 放宽到35
    signals['rsi_oversold'] = 1 if rsi_oversold else 0
    
    # 3. MACD金叉
    macd_cross = False
    if pd.notnull(prev['MACD']) and pd.notnull(prev['MACD_Signal']) and pd.notnull(latest['MACD']) and pd.notnull(latest['MACD_Signal']):
        macd_cross = (prev['MACD'] <= prev['MACD_Signal']) and (latest['MACD'] > latest['MACD_Signal'])
    signals['macd_cross'] = 1 if macd_cross else 0
    
    # 4. 布林带下轨
    bb_touch = False
    if pd.notnull(latest['close']) and pd.notnull(latest['BB_lower']):
        bb_touch = latest['close'] <= latest['BB_lower'] * 1.05  # 放宽到5%
    ...
    # 趋势确认
    trend_up = False
    if all(pd.notnull(latest[f'MA{period}']) for period in [5, 10, 20]):
        trend_up = (latest['MA5'] > latest['MA10'] > latest['MA20'])
    signals['trend_up'] = 1 if trend_up else 0
    
    # 放量确认
    volume_spike = False
    if len(data) >= 20 and pd.notnull(latest['volume']):
        volume_ma = data['volume'].rolling(20).mean().iloc[-1]
        if pd.notnull(volume_ma):
            volume_spike = latest['volume'] > volume_ma * 1.3
    signals['volume_spike'] = 1 if volume_spike else 0
...
def main():
    ...
    # 限制分析数量（演示用）
    sample_size = min(100, len(constituents))
...
def generate_markdown_report(results_df, constituents):
    ...
    report += "2. **RSI超卖策略**: RSI < 35，权重25%\n"
    report += "3. **MACD金叉策略**: DIF上穿DEA，权重25%\n"
    report += "4. **布林带策略**: 价格接近下轨（5%以内），权重20%\n"
    report += "5. **综合评分**: 加权求和，最高1分\n\n"
    
    report += "## 参数设置\n\n"
    report += "- MA周期: 5, 10, 20, 60日\n"
    report += "- RSI周期: 14日，超卖阈值: 35\n"
